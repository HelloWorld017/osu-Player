...
public class LyricLib {
	...
	public static Map<Double, String> parseLyric(InputStream is) throws SAXException, IOException, ParserConfigurationException{
		...
		TreeMap<Double, String> returnVal = new TreeMap<>();
		Arrays.stream(lyrics).forEach((rawLyric) -> {
			String[] split = rawLyric.split("\\]");
			if(split.length < 2) return;

			String[] timeStamps = split[0].split("\\.");
			String[] timeSplit = timeStamps[0].split(":");

			if(timeSplit.length < 2) return;

			String lyric = rawLyric.replace(split[0] + "]", "");
			double timeStamp = 
					Double.parseDouble(timeSplit[0].replace("[", "")) * 60 +
					Double.parseDouble(timeSplit[1]) +
					Double.parseDouble(timeStamps[1]) / 100;
